...
const TechStackSection: React.FC<TechStackProps> = ({
  ...
  const techCategories = [
    {
      ...
      technologies: [
        ...
        { name: "Next.js", icon: <Image src="/icons/nextjs.svg" className="w-12 h-12" width={24} height={24} alt="Next.js Icon" />, color: "text-gray-800" },
    ...
    },
    {
      ...
      technologies: [
        ...
        { name: "Express", icon: <Image src="/icons/express.svg" className="w-12 h-12" width={24} height={24} alt="Express Icon" />, color: "text-gray-700" },
    ...
    },
    {
      ...
      technologies: [
        ...
        { name: "Git", icon: <Image src="/icons/git.svg" className="w-12 h-12" width={24} height={24} alt="Git Icon" />, color: "text-gray-700" },
  ...
  return (
    <section ref={sectionRef} id="tech-stack" className="py-20 lg:py-20 bg-white overflow-hidden">
      <div className="max-w-6xl mx-auto px-8 lg:px-8">
        ...
        <div className={`text-center mb-16 transition-all duration-1000 ease-out ${
          ...
          <div className={`w-20 h-0.5 bg-black mx-auto mb-6 transition-all duration-1000 delay-300 ${
          ...
          {techCategories.map((category, categoryIndex) => (
            ...
            >
              ...
              <div className="text-center mb-12">
                <h3 className={`text-2xl font-medium text-black mb-2 transition-all duration-800 ${
                  isVisible ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'
                }`}
                style={{ transitionDelay: `${(categoryIndex + 1) * 250}ms` }}
                >
                  {category.category}
                </h3>
                <div className={`w-12 h-0.5 bg-gray-400 mx-auto transition-all duration-800 ${
                  isVisible ? 'scale-x-100' : 'scale-x-0'
                }`}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
                {category.technologies.map((tech, techIndex) => (
                  <div 
                    key={tech.name}
                    className={`group flex flex-col items-center space-y-4 p-8 bg-white hover:bg-gray-50 transition-all duration-500 border border-gray-100 hover:border-gray-200 hover:shadow-lg transform hover:-translate-y-2 ${
                      isVisible 
                        ? 'translate-y-0 opacity-100 scale-100 rotate-0' 
                        : 'translate-y-8 opacity-0 scale-90 rotate-1'
                    }`}
                    style={{ 
                      transitionDelay: `${(categoryIndex + 1) * 200 + techIndex * 100}ms`,
                      transformOrigin: 'center'
                    }}
                  >
                    {/* Tech Icon */}
                    <div className={`${tech.color} transition-all duration-500 group-hover:scale-110 ${
                    ...
                    {/* Tech Name */}
                    <h4 className={`text-lg font-medium text-black transition-all duration-500 text-center group-hover:text-gray-700 ${
                      isVisible ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'
                    }`}
                    style={{ transitionDelay: `${(categoryIndex + 1) * 200 + techIndex * 100 + 100}ms` }}
                    >
                      {tech.name}
                    </h4>
                  </div>
        ...
        >
          <div className="max-w-2xl mx-auto">
            <p className={`text-lg text-gray-600 mb-8 transition-all duration-800 ${
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {['Always Learning', 'Problem Solver', 'Team Player'].map((badge, index) => (
                <span 
                  key={badge}
                  className={`px-4 py-2 bg-white border border-gray-200 text-gray-700 text-sm font-medium hover:border-gray-300 hover:shadow-sm transition-all duration-400 ${
                    isVisible 
                      ? 'translate-y-0 opacity-100 scale-100' 
